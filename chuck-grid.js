...
(()=>{
  ...
  function render(){
    ...
    $('chuck_help').textContent=cornersOn?'點選格子輸入數值；四角點可一次開關。':'四角點已隱藏並保留原值；再次開啟即可顯示。';
    for(const id of corners){
      const input=$(id),cell=$('box_'+id);
      input.disabled=!cornersOn;
      cell.classList.toggle('off',!cornersOn);
      cell.toggleAttribute('inert',!cornersOn);
      if(cornersOn)cell.removeAttribute('aria-hidden');
